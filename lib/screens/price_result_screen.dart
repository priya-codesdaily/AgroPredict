...
class PriceResultScreen extends StatefulWidget {
  ...
  const PriceResultScreen({
    super.key, required this.cropName, required this.prices,
    this.isHindi = false, this.userLat, this.userLng,
  });
...
class _PriceResultScreenState extends State<PriceResultScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _speakResult(Map<String, dynamic> decision) async {
    ...
    if (widget.isHindi) {
      if (advice == 'SELL') {
        ...
        if (best != null) text += '${best.market} मंडी में सबसे अच्छा दाम ${best.modalPrice.toStringAsFixed(0)} रुपये है. ';
        if (nearest != null) { final p = nearest['price'] as CropPrice; text += 'आपके पास की मंडी ${p.market} है. '; }
        text += 'अभी बेचना सही रहेगा.';
      } else if (advice == 'WAIT') {
        text = '${widget.cropName} के लिए अभी इंतजार करना बेहतर है. ';
        if (nearest != null) { final p = nearest['price'] as CropPrice; text += '${p.market} में भाव ${p.modalPrice.toStringAsFixed(0)} रुपये है. '; }
        text += 'दाम बेहतर होने पर बेचें.';
      } else { text = '${widget.cropName} के दाम अभी एक जैसे हैं. कुछ दिन नज़र रखें.'; }
    } else {
      if (advice == 'SELL') {
        text = 'Good time to sell ${widget.cropName}. ';
        if (best != null) text += '${best.market} has the best price at ${best.modalPrice.toStringAsFixed(0)} rupees. ';
        if (nearest != null) { final p = nearest['price'] as CropPrice; text += 'Nearest mandi is ${p.market}. '; }
        text += 'Sell now.';
      } else if (advice == 'WAIT') {
        text = 'Better to wait before selling ${widget.cropName}. ';
        if (nearest != null) { final p = nearest['price'] as CropPrice; text += '${p.market} price is ${p.modalPrice.toStringAsFixed(0)} rupees. '; }
        text += 'Monitor before selling.';
      } else { text = 'Prices for ${widget.cropName} are similar. Monitor for a few more days.'; }
    }
    await _tts.setLanguage(widget.isHindi ? 'hi-IN' : 'en-IN');
  ...
  Widget build(BuildContext context) {
    ...
    if (_selectedMandiForCalc == null && best != null) _selectedMandiForCalc = best;
        ...
        actions: [
          GestureDetector(
            ...
            child: Container(
              ...
              child: Text(_showPerKg ? (widget.isHindi ? '₹/किलो' : '₹/kg') : (widget.isHindi ? '₹/क्विंटल' : '₹/qtl'),
        ...
        children: [
          ...
          SingleChildScrollView(
            ...
            child: Column(children: [
              ...
              if (widget.userLat != null && widget.userLng != null) ...[_buildNearestMandiCard(), const SizedBox(height: 16)],
              Container(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [
                    ...
                    const SizedBox(width: 8),
                    Text(widget.isHindi ? 'यह सलाह क्यों?' : 'WHY THIS ADVICE?', style: TextStyle(color: ac, fontSize: 11, fontWeight: FontWeight.bold, letterSpacing: 1.2)),
                  ]),
                  const SizedBox(height: 12),
                  ...
                  child: Column(children: [
                    Row(children: [
                      ...
                      Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                        Text('₹${_getPrice(best.modalPrice).toStringAsFixed(_showPerKg ? 1 : 0)}', style: const TextStyle(color: Color(0xFF52B788), fontSize: 24, fontWeight: FontWeight.w900)),
                    ...
                    Container(
                      ...
                      child: Text(
                        widget.isHindi ? '👉 यहाँ बेचने पर औसत से ₹${_getPrice(diff).toStringAsFixed(0)} ज़्यादा मिलेगा' : '👉 Sell here to earn ₹${_getPrice(diff).toStringAsFixed(0)} more than average',
              ...
              Container(
                ...
                child: Row(children: [
                  ...
                  const SizedBox(width: 8),
                  Expanded(child: Text(
                    widget.isHindi ? '$count मंडियों के आधार पर · AGMARKNET${lastUpdate.isNotEmpty ? " · $lastUpdate" : ""}' : 'Based on $count mandis · AGMARKNET Govt Data${lastUpdate.isNotEmpty ? " · Updated $lastUpdate" : ""}',
          ...
          StatefulBuilder(builder: (context, setTabState) {
            ...
            return SingleChildScrollView(
              padding: const EdgeInsets.all(20),
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(widget.isHindi ? 'मंडी चुनें — जानें यात्रा फायदेमंद है या नहीं' : 'Pick a mandi — know if travel is worth it', style: TextStyle(color: Colors.white.withOpacity(0.6), fontSize: 13)),
                const SizedBox(height: 12),
                Container(
                  ...
                  child: DropdownButtonHideUnderline(
                    child: DropdownButton<CropPrice>(
                      value: _selectedMandiForCalc, isExpanded: true, dropdownColor: const Color(0xFF1A2744),
                      style: const TextStyle(color: Colors.white, fontSize: 14),
                      hint: Text(widget.isHindi ? 'मंडी चुनें' : 'Select a mandi', style: TextStyle(color: Colors.white.withOpacity(0.4))),
                      items: widget.prices.map((p) {
                        ...
                        return DropdownMenuItem(value: p, child: Row(children: [
                          ...
                          Text('₹${_getPrice(p.modalPrice).toStringAsFixed(0)}', style: const TextStyle(color: Color(0xFF52B788), fontWeight: FontWeight.bold)),
                        ]));
                      }).toList(),
                      onChanged: (val) { setState(() => _selectedMandiForCalc = val); setTabState(() {}); },
                    ),
                  ),
                ),
                if (autoDist != null) ...[
                  ...
                  Container(
                    ...
                    child: Row(children: [
                      ...
                      const SizedBox(width: 8),
                      Expanded(child: Text(
                        widget.isHindi ? 'अनुमानित दूरी: ${autoDist.toStringAsFixed(0)} किमी · मात्रा डालें और लाभ देखें' : 'Estimated distance: ${autoDist.toStringAsFixed(0)} km · Enter quantity to calculate profit',
                  ...
                  Container(
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        ...
                        Text(widget.isHindi ? 'परिवहन लाभ कैलकुलेटर' : 'TRANSPORT PROFIT CALCULATOR', style: const TextStyle(color: Color(0xFF52B788), fontSize: 11, fontWeight: FontWeight.bold, letterSpacing: 1.2)),
                      ]),
                      const SizedBox(height: 8),
                      Text('${_selectedMandiForCalc!.market} — ₹${_getPrice(_selectedMandiForCalc!.modalPrice).toStringAsFixed(_showPerKg ? 1 : 0)} ${_unit()}', style: const TextStyle(color: Color(0xFF52B788), fontSize: 12)),
                      const SizedBox(height: 12),
                      ...
                      TextField(controller: qtyC, keyboardType: TextInputType.number, style: const TextStyle(color: Colors.white),
                        decoration: InputDecoration(
                          hintText: widget.isHindi ? 'मात्रा (क्विंटल में)' : 'Quantity in quintals',
                          hintStyle: TextStyle(color: Colors.white.withOpacity(0.3), fontSize: 13),
                          filled: true, fillColor: const Color(0xFF0A1628),
                          contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 16),
                          border: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: BorderSide.none),
                      ...
                      GestureDetector(
                        onTap: () {
                          ...
                          if (dist == 0 || qty == 0) { resultN.value = widget.isHindi ? 'कृपया दूरी और मात्रा दर्ज करें' : 'Please enter distance and quantity'; return; }
                          final r = DecisionEngine.calculateProfit(price: _selectedMandiForCalc!.modalPrice, quantity: qty, distance: dist, avgPrice: avg);
                          bool wi = r['worthIt'] as bool;
                          double ng = r['netGain'] as double;
                          double tc = r['transportCost'] as double;
                          double ex = r['extraEarnings'] as double;
                          resultN.value = wi
                              ? (widget.isHindi ? 'यात्रा फायदेमंद है ✅\n\nशुद्ध लाभ: ₹${ng.toStringAsFixed(0)}\nअतिरिक्त कमाई: ₹${ex.toStringAsFixed(0)}\nयात्रा खर्च: ₹${tc.toStringAsFixed(0)}' : 'Worth the travel ✅\n\nNet gain: ₹${ng.toStringAsFixed(0)}\nExtra earnings: ₹${ex.toStringAsFixed(0)}\nTravel cost: ₹${tc.toStringAsFixed(0)}')
                              : (widget.isHindi ? 'यात्रा फायदेमंद नहीं ❌\n\nयात्रा खर्च: ₹${tc.toStringAsFixed(0)}\nनज़दीकी मंडी में बेचना बेहतर है' : 'Not worth the travel ❌\n\nTravel cost: ₹${tc.toStringAsFixed(0)}\nSell at nearest mandi instead');
                        },
              ...
              Container(
                ...
                child: BarChart(BarChartData(
                  ...
                  titlesData: FlTitlesData(
                    leftTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, reservedSize: 45, getTitlesWidget: (v, m) => Text('₹${_getPrice(v).toStringAsFixed(0)}', style: const TextStyle(color: Colors.white38, fontSize: 8)))),
                    ...
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        ...
                        if (p == best) Container(
                          ...
                          child: Text(widget.isHindi ? '✅ सबसे अच्छा' : '✅ BEST', style: const TextStyle(color: Color(0xFF52B788), fontSize: 9, fontWeight: FontWeight.bold)),
                        ),
                      ]),
                      Text(p.district, style: TextStyle(color: Colors.white.withOpacity(0.4), fontSize: 12)),
                      if (dist != null) Text('📍 ~${dist.toStringAsFixed(0)} km', style: const TextStyle(color: Colors.blueAccent, fontSize: 11)),
                    ])),
                    Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                      Text('₹${_getPrice(p.modalPrice).toStringAsFixed(_showPerKg ? 1 : 0)}', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
                      Text(_unit(), style: TextStyle(color: Colors.white.withOpacity(0.4), fontSize: 10)),
                    ]),
                  ]),
                );
      ...
      return Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            const SizedBox(width: 8),
            Text(widget.isHindi ? '⚡ क्या यात्रा फायदेमंद है?' : '⚡ IS TRAVEL WORTH IT?', style: const TextStyle(color: Color(0xFF52B788), fontSize: 11, fontWeight: FontWeight.bold, letterSpacing: 1)),
          ]),
          const SizedBox(height: 10),
          Row(children: [
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (dist != null) Text('📍 ~${dist.toStringAsFixed(0)} km', style: const TextStyle(color: Colors.blueAccent, fontSize: 11))
              else Text(widget.isHindi ? 'लाभ टैब में दूरी डालें' : 'Enter distance in Profit tab', style: TextStyle(color: Colors.white.withOpacity(0.3), fontSize: 11)),
            ])),
            SizedBox(
              width: 110,
              child: TextField(
                ...
                onChanged: (val) {
                  ...
                  double d = dist ?? 0;
                  if (qty > 0 && d > 0) {
                    final r = DecisionEngine.calculateProfit(price: best.modalPrice, quantity: qty, distance: d, avgPrice: avg);
                    bool wi = r['worthIt'] as bool;
                    double ng = r['netGain'] as double;
                    double tc = r['transportCost'] as double;
                    resultN.value = '${wi ? "✅" : "❌"}|${ng.toStringAsFixed(0)}|${tc.toStringAsFixed(0)}';
                    ss(() {});
                  }
                },
                decoration: InputDecoration(
              ...
              return Container(
                ...
                child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(pos ? (widget.isHindi ? '✅ यात्रा फायदेमंद है' : '✅ Worth the travel') : (widget.isHindi ? '❌ फायदेमंद नहीं' : '❌ Not worth it'),
                        style: TextStyle(color: pos ? const Color(0xFF52B788) : Colors.redAccent, fontSize: 12, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 4),
                    Text(widget.isHindi ? 'यात्रा खर्च: ₹$cost' : 'Travel cost: ₹$cost', style: TextStyle(color: Colors.white.withOpacity(0.5), fontSize: 11)),
                  ]),
                  Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                    Text('${pos ? "+" : ""}₹$gain', style: TextStyle(color: pos ? const Color(0xFF52B788) : Colors.redAccent, fontSize: 22, fontWeight: FontWeight.w900)),
                    Text(widget.isHindi ? 'शुद्ध लाभ' : 'net gain', style: TextStyle(color: Colors.white.withOpacity(0.4), fontSize: 10)),
                  ]),
                ]),
              );
            },
  ...
  Widget _buildNearestMandiCard() {
    ...
    String distText(double? d) => d != null ? '~${d.toStringAsFixed(0)} km ${widget.isHindi ? "दूर" : "away"}' : (widget.isHindi ? 'दूरी अज्ञात' : 'Distance unknown');
    String travelText(double? d) { if (d == null) return ''; int h = (d / 60).floor(); int m = ((d / 60 - h) * 60).round(); return h > 0 ? '~$h hr ${m}min' : '~${m}min'; }
      ...
      Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            const SizedBox(width: 8),
            Text(widget.isHindi ? '📍 आपके पास की मंडी' : '📍 NEAREST MANDI', style: const TextStyle(color: Colors.blueAccent, fontSize: 11, fontWeight: FontWeight.bold, letterSpacing: 1)),
          ]),
          const SizedBox(height: 12),
          Row(children: [
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(nearestPrice.market, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
              Text('${nearestPrice.district}, ${nearestPrice.state}', style: TextStyle(color: Colors.white.withOpacity(0.5), fontSize: 12)),
              ...
              Row(children: [
                ...
                if (travelText(nearestDist).isNotEmpty) ...[const SizedBox(width: 8), const Icon(Icons.access_time, color: Colors.blueAccent, size: 14), const SizedBox(width: 4), Text(travelText(nearestDist), style: const TextStyle(color: Colors.blueAccent, fontSize: 12))],
              ]),
            ])),
            Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
              Text('₹${_getPrice(nearestPrice.modalPrice).toStringAsFixed(_showPerKg ? 1 : 0)}', style: const TextStyle(color: Colors.blueAccent, fontSize: 22, fontWeight: FontWeight.w900)),
              Text(_unit(), style: TextStyle(color: Colors.white.withOpacity(0.4), fontSize: 10)),
            ]),
          ]),
        ]),
      ...
      if (bestPrice != null && bestPrice.market != nearestPrice.market) ...[
        ...
        Container(
          ...
          child: Row(children: [
            ...
            const SizedBox(width: 8),
            Expanded(child: Text(
              widget.isHindi ? '💡 ${bestPrice.market} में ज़्यादा दाम (${distText(bestDist)}) — लाभ टैब में देखें' : '💡 ${bestPrice.market} has higher price (${distText(bestDist)}) — check Profit tab',
